...
def validate_operational_tables(
    ...
    for index, row in enumerate(tables["transport.csv"], 2):
        ...
        for field in ("duration_minutes", "buffer_minutes", "planned_cost_cny"):
            # Research drafts may keep a scalar blank when notes explain an
            # estimated range. Do not convert an unknown fare into zero/free.
            draft_range = not publish and row["status"] == "candidate" and bool(row["notes"].strip())
            value = number(row[field], f"transport.csv 第 {index} 行 {field}", report, allow_blank=draft_range)
            if value < 0:
    ...
    for index, row in enumerate(tables["bookings.csv"], 2):
        ...
        cost = number(
            ...
            report,
            allow_blank=not publish and row["status"] in {"monitor", "unknown", "not_required"} and bool(row["notes"].strip()),
        )
        if cost < 0:
