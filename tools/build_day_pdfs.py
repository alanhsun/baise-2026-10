...
DAY_DATES = {i: f"2026-10-{i:02d}" for i in range(1, 7)}
...
def build_one(project: Path, output: Path, day_number: int, route: dict, styles):
    ...
    doc = SimpleDocTemplate(
        ...
        author="百色靖西家庭旅行", subject="每日离线行程备查",
    )
    title_parts = title.split("｜", 1)
    if len(title_parts) == 2:
        story = [Spacer(1, 4 * mm), Paragraph(inline_markup(title_parts[0]), styles["cover_sub"]),
...
def main() -> int:
    ...
    overview_pdf = project / "tmp" / "overview-pages.pdf"
    ...
    combined = PdfWriter()
    combined.append(str(overview_pdf), outline_item="总览、交通、酒店与预算")
    for index, path in enumerate(built, 1):
        combined.append(str(path), outline_item=f"D{index} · 10/{index}")
    full = output / "baise-family-guide.pdf"
    combined.add_metadata({"/Title":"百色与靖西六日家庭旅行攻略 · 打车版", "/Author":"家庭旅行规划"})
    with full.open("wb") as f:
        combined.write(f)
    for path in built + [full]:
        print(path)
    return 0
