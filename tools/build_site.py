...
STATUS_LABELS = {
    "candidate": "候选", "estimate": "估算", "confirmed": "已确认",
    "selected": "已订",
    "ready": "待办理",
    "monitor": "待复核",
    "reserved": "已预约",
...
def render_budget(rows: list[dict[str, str]]) -> tuple[str, float, float, float]:
    ...
    labels = {
        ...
        "planned_high_cny": "预计高值", "actual_cny": "已支付",
    }
...
def main() -> int:
    ...
    active_transport = [row for row in tables["transport.csv"] if row.get("status") in {"confirmed", "candidate"}]
    active_accommodation = [row for row in tables["accommodation.csv"] if row.get("status") in {"selected", "candidate"}]
    active_bookings = [
...
"""
    ...
    if output != project / "dist":
        raise ValueError("Only project-local dist output is supported")
    ...
    media = project / "media"
    if media.is_dir():
        shutil.copytree(media, output / "media", dirs_exist_ok=True)
    daily_pdfs = project / "output" / "pdf"
    if daily_pdfs.is_dir():
        shutil.copytree(daily_pdfs, output / "pdf", dirs_exist_ok=True)
